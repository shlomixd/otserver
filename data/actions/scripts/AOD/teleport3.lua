  local config =
{
	...
	rebornN = 3 -- reborn value requirement
}
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if getPlayerStorageValue(cid, config.rebornV) >= config.rebornN then -- If players "reborn" storage is greater than RebornN. THEN:
		...
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Welcome to Troll Champions Spawn.")
	else
		doTeleportThing(cid, config.posWrong) -- reject their access.
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Sorry, you need 3reborns to enter.")
	end
	return true
end
