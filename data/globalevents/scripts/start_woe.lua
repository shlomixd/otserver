	...
	if isInTable(config.dias, day) == true and lastday ~= day then
		...
		if Hora == config.horaPartida then
			if not Woe.isTime() then
				doBroadcastMessage("War of Emperium will start in next 5 minutes..Teleport at temple!", config.bcType)
				doBroadcastMessage("Woe will have a duration of " .. config.timeToEnd .. " minutes.", config.bcType)
